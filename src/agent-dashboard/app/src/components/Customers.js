        ...
        <div className="helpdesk-main">
            <div className="helpdesk-customers" style={{ width: '100%' }}>
                {customer && customer.map((customer) => {
                    return (
                        <div key={customer.id} className="helpdesk-customer">
                            <Link to={`/customer/${customer.id}`}>
                                <h4 className="primary" style={{ margin: '5px 0', fontSize: '16px' }}>{customer.name}</h4>
                            </Link>
                            <div className="helpdesk-customer-email" style={{ marginBottom: '5px' }}>{customer.email}</div>
                        </div>
                    )
                })}
